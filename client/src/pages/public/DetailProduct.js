...
const DetailProduct = ({ isQuickView, data, location, dispatch, navigate }) => {
  ...
  const fetchProducts = async () => {
    const response = await getProductsSortByRating();
    if (response.success) {
      setRelatedProducts(response.productData?.slice(0, 6) ?? []);
    }
  };
  useEffect(() => {
    if (pid) {
  ...
  return (
    <div className={clsx("w-full")}>
      ...
      >
        ...
        >
          <div className="w-[458px] h-[458px] border flex items-center overflow-hidden justify-center">
            <ReactImageMagnify
              {...{
